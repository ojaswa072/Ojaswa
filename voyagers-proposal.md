# Voyager's 2.0 — Proposed Organizational Structure & Founding Leadership Selection

**Status:** Proposal for Coordinator Review  

---

## 1. Organizational Overview

Voyager's 2.0 is proposed to operate through:

- **1 Core Council** consisting of 3 positions.
- **7 equal departments**, each with the same internal hierarchy.
- A simple chain of responsibility:

> **Department Head → Secretaries → Coordinators → Members**


### Proposed Departments

1. Technical
2. Research & Open Source
3. Cultural
4. Sports
5. Editorial
6. Design & Media
7. Corporate Relations

---

# 2. Core Council

The Core Council provides overall leadership, administration, coordination, and financial oversight for Voyager.

## 2.1 President

**Primary responsibility:** Overall leadership and representation of Voyager.

### Responsibilities
- Sets the overall vision and direction.
- Represents Voyager before college administration and external stakeholders.
- Chairs Core Council meetings.
- Oversees all departments.
- Approves major initiatives in coordination with the Core Council.
- Ensures the organization works toward its annual objectives.

---

## 2.2 Vice President

**Primary responsibility:** Overall coordination, execution, and financial oversight.

### Responsibilities
- Coordinates between all seven departments.
- Monitors progress of major initiatives.
- Resolves inter-department coordination issues.
- Supports the President in organizational decisions.
- Ensures decisions of the Core Council are implemented.
- Oversees financial management and maintains financial records.
- Coordinates budget preparation and tracks expenditure and reimbursements.
- Maintains sponsorship-related financial records.
- Ensures spending follows college/institutional rules.

---

## 2.3 General Secretary

**Primary responsibility:** Administration, documentation, and official coordination.

### Responsibilities
- Maintains official records.
- Prepares meeting agendas and minutes.
- Handles official correspondence.
- Maintains the organizational calendar.
- Maintains membership and leadership records.
- Maintains the central documentation repository.
- Coordinates official notices and internal communication.

> **Note:** The title **General Secretary** is retained to clearly distinguish this Core Council role from department-specific Secretaries.

---

# 3. Department Structure

Every department follows the same hierarchy:

> **1 Head → 2 Secretaries → 4 Coordinators → Members**

### Role definitions

**Department Head**
- Owns the overall direction and performance of the department.
- Plans departmental activities.
- Coordinates with the Core Council.
- Delegates responsibilities to Secretaries.
- Ensures departmental objectives are achieved.

**Secretary**
- Owns a defined functional area within the department.
- Plans and manages activities related to that functional area.
- Works with Coordinators to execute initiatives.
- Reports progress to the Department Head.

**Coordinator**
- Executes specific activities, projects, or events.
- Coordinates volunteers/members for assigned activities.
- Handles operational tasks and reports progress to the relevant Secretary.

**Member**
- Participates in and contributes to departmental activities, projects, and events.

> **Important:** The proposed numbers are organizational targets, not rigid limits. Department membership can expand according to workload and actual requirements.

---

# 4. Technical Department

## Technical Head

**Primary responsibility:** Overall technical direction and activities of the department.

### Secretary 1 — Development & Projects
- Web/app/software development
- Technical projects
- Development workshops
- Project-based learning

### Secretary 2 — Competitive Programming & DSA
- Coding contests
- DSA sessions
- Competitive programming
- Coding practice initiatives

### Coordinators - 4

### Members
Participate in technical activities, projects, competitions, and learning initiatives.

---

# 5. Research & Open Source Department

## Research & Open Source Head

**Primary responsibility:** Building a research- and contribution-oriented culture.

### Secretary 1 — Research & Publications
- Research projects
- Paper reading
- Academic writing
- Publication guidance

### Secretary 2 — Open Source
- GitHub projects
- Open-source contribution
- Issue/PR activities
- Open-source community participation

### Coordinators - 4

### Members
Participate in research, open-source projects, innovation initiatives, and collaborative work.

---

# 6. Cultural Department

## Cultural Head

**Primary responsibility:** Cultural activities and representation of Voyager.

### Secretary 1 — Music
- Singing
- Instrumental activities
- Music competitions
- Music performances

### Secretary 2 — Dance & Performance
- Dance activities
- Choreography
- Performances
- Dance competitions

### Coordinators - 4

### Members
Participate in performances, competitions, and cultural activities.

---

# 7. Sports Department

## Sports Head

**Primary responsibility:** Sports, games, and physical activity initiatives.

### Secretary 1 — Outdoor Sports
- Cricket
- Football
- Volleyball
- Athletics
- Other outdoor sports

### Secretary 2 — Indoor Sports
- Chess
- Carrom
- Table tennis
- Other indoor games

### Coordinators - 4

### Members
Participate in sports teams, tournaments, and activities.

---

# 8. Editorial Department

## Editorial Head

**Primary responsibility:** Voyager's written, literary, and intellectual content.

### Secretary 1 — Content & Writing
- Articles
- Blogs
- Feature stories
- Creative writing

### Secretary 2 — Publications & Newsletter
- Newsletter
- Club magazine
- Event reports
- Publications

### Coordinators - 4

### Members
Write, edit, research, and contribute to editorial and literary activities.

---

# 9. Design & Media Department

## Design & Media Head

**Primary responsibility:** Voyager's visual identity and media production.

### Secretary 1 — Graphic Design & Branding
- Posters
- Social media creatives
- Event branding
- Visual identity

### Secretary 2 — Photography & Videography
- Event photography
- Event videography
- Interviews
- Visual documentation

### Coordinators - 4

### Members
Support design, photography, videography, and media production.

**Operational principle:** This department handles content production. Decisions regarding official organizational communication/publication remain with the relevant department leadership/Core Council. This separates creative execution from organizational authority.

---

# 10. Corporate Relations Department

## Corporate Relations Head

**Primary responsibility:** External relationships and institutional/corporate partnerships.

### Secretary 1 — Sponsorships
- Identify potential sponsors
- Sponsorship proposals
- Sponsor communication
- Sponsorship deliverables

### Secretary 2 — Industry Relations
- Company connections
- Industry professionals
- Guest speakers
- Industry sessions

### Coordinators - 4

### Members
Research prospects, support outreach, prepare proposals, and assist with external coordination.

**Financial principle:** Corporate Relations may negotiate and coordinate sponsorships, but financial commitments, receipts, expenditure, and financial records remain under the Vice President/Core Council.

---

# 11. Complete Organizational Hierarchy

```text

VOYAGER'S 2.0
                              │
                       ┌──────┴──────┐
                       │ CORE COUNCIL│
                       └──────┬──────┘
                              │
         ┌────────────────────┼────────────────────┐
         │                    │                    │
     President          Vice President     General Secretary
                              │            Administration
                              │            Documentation
                              │            Official Coordination
                              │
                    Financial Management
                              │
                              ▼
                    7 EQUAL DEPARTMENTS
                              │
       ┌────────┬─────────────┼────────────┬────────┬────────┬────────────┐
       │        │             │            │        │        │            │
   Technical Research     Cultural     Sports   Editorial Design &    Corporate
             & Open                                Media     Relations
             Source
       │        │             │            │        │        │            │
     Head     Head           Head         Head     Head     Head         Head
       │        │             │            │        │        │            │
    2 Sec.   2 Sec.         2 Sec.       2 Sec.   2 Sec.   2 Sec.       2 Sec.
       │        │             │            │        │        │            │
  4 Coord. 4 Coord.         4 Coord.     4 Coord. 4 Coord. 4 Coord.     4 Coord.
       │        │             │            │        │        │            │
   Members   Members        Members      Members  Members  Members      Members

``` 






## 12. Position Count

**Per Department**
- 1 Head
- 2 Secretaries
- 4 Coordinators
- 4 initial Members

**Total = 11 positions per department**

**Across 7 Departments**
- 7 × 11 = 77 department positions

**Including Core Council**
- 77 Department positions
- 3 Core Council positions

**Total proposed organizational capacity = 80 positions**

---

## 13. Founding Leadership Team — Selection Process

The initial objective is to establish a Founding Leadership Team of 17 fourth-year students before opening recruitment to other years.

**Proposed composition:**
- 3 Core Council positions
- 7 Department Heads
- 7 Department Secretaries

**Total = 17 founding leaders**

The initial founding team is therefore not intended to fill all 77 department positions immediately. The remaining Secretaries, Coordinators, and Members can be recruited after the founding structure is established.

---

### 13.1 Eligibility

The initial Founding Leadership Team application will be open to 4th-year students.

After the founding leadership team is finalized, recruitment can be opened to students from other eligible years for the remaining departmental positions.

---

## 14. Founding Leadership Application

A single application form will be used for all 4th-year applicants rather than creating separate application processes for each position.

**Section 1 — Basic Information**
- Name
- Branch
- Contact information
- Previous club/organizational experience

---

**Section 2 — Leadership Interest**

Which leadership area interests you?
- Core Council
- Department Head
- Department Secretary
- Open to any suitable leadership position

If interested in Core Council, which position(s) interest you?
- President
- Vice President
- General Secretary
- Open to any Core Council position

**Department Preferences**
Applicants selecting a department-level leadership role will provide two department preferences:

*First preference:*
- Technical
- Research & Open Source
- Cultural
- Sports
- Editorial
- Design & Media
- Corporate Relations

*Second preference:*
- Technical
- Research & Open Source
- Cultural
- Sports
- Editorial
- Design & Media
- Corporate Relations

The preferences are intended to guide placement while allowing the selection panel to assign the most suitable role based on the applicant's skills, experience, interview performance, and organizational requirements.

---

## 15. Section 3 — Skills & Experience

Applicants will provide:
- Relevant skills
- Previous responsibilities
- Projects handled
- Events organized
- Club/organizational experience
- Any other experience relevant to the role

A useful experience question:
> Describe one project, event, activity, or responsibility that you independently handled. What was your role and what was the outcome?

This focuses on demonstrated ownership rather than simply asking whether an applicant has "leadership experience."

---

## 16. Section 4 — Motivation

> Why do you want to be part of the Founding Leadership Team of Voyager's 2.0, and what would you like to contribute or build through it?

---

## 17. Section 5 — Availability & Commitment

Applicants will indicate their realistic weekly availability.

Suggested options:
- 1–2 hours
- 2–4 hours
- 4–6 hours
- 6+ hours

The purpose is to understand realistic commitment, particularly for leadership positions.

---

## 18. Selection Process

The proposed selection process is:

**Step 1 — Application**
Open the Founding Leadership Team form to 4th-year students.

**Step 2 — Application Review**
Review submitted applications for relevant experience, skills, motivation, preferences, and availability.

**Step 3 — Interview**
Invite suitable applicants for a structured interview.

Interviews should focus on:
- Leadership ability
- Reliability and ownership
- Communication
- Problem solving
- Relevant skills
- Department-specific understanding
- Commitment
- Ability to work with others

Core Council candidates may receive additional questions because their responsibilities affect the entire organization.

**Step 4 — Role Allocation**
A selection panel reviews candidates and determines the most suitable placement.

The final allocation will consider:
1. Demonstrated capability
2. Leadership potential
3. Relevant skills and experience
4. Interview performance
5. Stated preferences
6. Commitment and availability
7. Overall organizational requirements

The applicant's preference is considered, but it does not automatically guarantee a particular position.

**Step 5 — Finalize the 17-Person Founding Leadership Team**
The selected team will consist of:

*Core Council*
- President
- Vice President
- General Secretary

*Department Leadership*
- 7 Department Heads
- 7 Department Secretaries

**Step 6 — Open Recruitment**
After the founding leadership structure is finalized, recruitment will be opened to all eligible years for the remaining positions.

The subsequent structure will develop toward:
> Head → Secretaries → Coordinators → Members
